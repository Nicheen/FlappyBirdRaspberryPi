...
class Pipe {
    ...
    draw(ctx, canvasHeight, nightMode = false) {
    const pipeWidth = 50;
    ...
    if (pipeImg.complete && pipeImg.naturalWidth > 0) {
        // bottom pipe
        ctx.drawImage(pipeImg, this.x, gapBottom, drawWidth, drawHeight);
        // top pipe flipped
        ctx.save();
        ctx.scale(1, -1);
        ctx.drawImage(pipeImg, this.x, -gapTop, drawWidth, drawHeight);
        ctx.restore();
    } else {
       
        ctx.fillStyle = "green";
        ctx.fillRect(this.x, 0, drawWidth, gapTop);
        ctx.fillRect(this.x, gapBottom, drawWidth, canvasHeight - gapBottom - 50);
    }
    }
...
class Pipes {
    ...
    draw(ctx, canvasHeight, nightMode = false) {
        this.pipeList.forEach(pipe => pipe.draw(ctx, canvasHeight, nightMode));
    }
...
class Game {
    constructor() {
        ...
        this.backgroundImageNight = new Image();//c
        this.backgroundImageNight.src = path_image_background_night;//c
    ...
    update(deltaTime) {
        ...
        if(this.bird.score >= 10) {//night vision mode on after passing pipes 10
          this.nightMode = true;
         }
    ...
    //Improved background loading check
    if (this.nightMode) {
    ...
    // draw pipes
    this.pipes.draw(this.ctx, this.canvas.height, this.nightMode);
    //draw bird
...
const path_image_background_night = "./images/nightbackground.jpg";//change
const path_image_pipe_night       = "./images/pipenight.png";//change
